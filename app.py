...
def changeProxy(proxy_group):
    ...
    while True:
        ...
        try:
            response = requests.get('https://pro.ip-api.com/json/?key=niMr70clVRi33Yx', proxies=proxies,  timeout=15, verify=False)
...
def authorize_account(proxy_group,login,password,android,phone_id,device_id,adid_id,session_id,mid,rur,ds_user_id,claim,csrftoken,authorization,sessionid):
    proxies,accept_ln,ig_locale,ig_locale_startup,timezone,timezonename = changeProxy(proxy_group)
    ...
    while True:
        try:
            ...
            response = requests.get('https://i.instagram.com/api/v1/accounts/current_user/?edit=true', headers=headers, timeout=maxtimeout, proxies=proxies, verify=False)
...
def check_validity_thread(account_queue, result_queue, status_queue, proxy_group):
    ...
    while not account_queue.empty():
        ...
        login,password,device,cookie = massiveAcc
        ...
        #authorize_account
        login,password,android,phone_id,device_id,adid_id,session_id,mid,rur,ds_user_id,claim,csrftoken,authorization,sessionid = authorize_account(proxy_group,login,password,android,phone_id,device_id,adid_id,session_id,mid,rur,ds_user_id,claim,csrftoken,authorization,sessionid)
...
class MainWindow(QMainWindow):
    ...
    def check_validity(self, table, items, proxy_group, processesx, threads):
        ...
        for i in range(0, total_accounts, accounts_per_process):
            ...
            login_list = [
                (table.item(row, 0).text(), table.item(row, 1).text(), table.item(row, 2).text(), table.item(row, 3).text())
                for row in chunk
            ]
            row_list = chunk
            for row in chunk:
